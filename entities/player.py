...
class Player:
    ...
    def __init__(self, start_position: Vector2, start_size: float = PLAYER_START_SIZE):
        ...
        self.collision_rect = pygame.Rect(
            start_position.x - start_size,
            start_position.y - start_size,
            start_size * 2,
            start_size * 2,
        )
        ...
        if not self.is_split:
            ...
            for blob in self.split_blobs:
                if blob.is_active and blob.size >= PLAYER_MIN_SIZE * 2:
                    ...
                    # Create 2 new blobs
                    for i in range(2):
                        angle = blob.split_angle + (
                            math.pi * i
                        )  # Offset from current angle
                        offset_distance = blob.size * 1.2

                        new_blob = SplitBlob(
                            position=Vector2(0, 0), size=split_size, color=self.color
                        )

                        new_blob.set_split_position(
                            blob.position, angle, offset_distance
                        )
    ...
    def eat_food(self, food) -> bool:
        ...
        if self.is_split:
            ...
            growth_value = calculate_growth_value(food.score_value, self.size)
    ...
    def check_collision_with_food(self, food) -> bool:
        ...
        if self.is_split:
            ...
            return any(
                blob.check_collision_with_food(food) for blob in self.split_blobs
            )
        else:
    ...
    def check_collision_with_enemy(self, enemy) -> bool:
        ...
        if self.is_split:
            # Check collision with any split blob
            return any(
                blob.check_collision_with_enemy(enemy) for blob in self.split_blobs
            )
        else:
            # Check collision with main player
            return distance_between_points(self.position, enemy.position) <= (
