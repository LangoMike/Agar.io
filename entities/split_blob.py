...
class SplitBlob:
    ...
    def eat_enemy(self, enemy):
        """Consume enemy and grow with diminishing returns"""
        if self.can_eat_enemy(enemy):
            base_growth = enemy.size * 0.8  # Get 80% of enemy mass
    ...
    def eat_player_blob(self, other_blob):
        """Consume another player blob and grow with diminishing returns"""
        if self.can_eat_player_blob(other_blob):
            base_growth = other_blob.size * 0.9  # Get 90% of blob mass
            growth_value = calculate_growth_value(base_growth, self.size)
            self.size += growth_value
            self.speed = self._calculate_speed()
            self._update_collision_properties()
    ...
    def check_collision_with_enemy(self, enemy) -> bool:
        ...
        return circles_intersect(self.position, self.size, enemy.position, enemy.size)
    ...
    def check_collision_with_player_blob(self, other_blob) -> bool:
        ...
        return circles_intersect(
            self.position, self.size, other_blob.position, other_blob.size
        )
