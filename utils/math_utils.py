...
def point_in_circle(point: Vector2, circle_center: Vector2, circle_radius: float) -> bool:
    """Check if a point is inside a circle"""
    return distance_between_points(point, circle_center) <= circle_radius

def circles_intersect(center1: Vector2, radius1: float, center2: Vector2, radius2: float) -> bool:
...
def calculate_zoom_factor(player_size: float, base_size: float = 20, min_zoom: float = 0.15, power: float = 0.2) -> float:
    """Calculate camera zoom factor based on player size"""
    return max(min_zoom, (base_size / player_size) ** power)

def world_to_screen(world_pos: Vector2, camera_x: float, camera_y: float, zoom_factor: float) -> Vector2:
    """Convert world coordinates to screen coordinates"""
    return Vector2(
        (world_pos.x - camera_x) * zoom_factor,
        (world_pos.y - camera_y) * zoom_factor
    )

def screen_to_world(screen_pos: Vector2, camera_x: float, camera_y: float, zoom_factor: float) -> Vector2:
    """Convert screen coordinates to world coordinates"""
    return Vector2(
        (screen_pos.x / zoom_factor) + camera_x,
        (screen_pos.y / zoom_factor) + camera_y
    )

def calculate_effective_screen_dimensions(screen_width: int, screen_height: int, zoom_factor: float) -> Tuple[int, int]:
